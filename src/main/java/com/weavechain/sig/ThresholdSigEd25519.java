...
public class ThresholdSigEd25519 {
    ...
    private static final ThreadLocal<Map<Set<Integer>, List<Scalar>>> cachedCoef =
            ThreadLocal.withInitial(ConcurrentHashMap::new);

    private static final ThreadLocal<SecureRandom> RANDOM = ThreadLocal.withInitial(SecureRandom::new);

    public ThresholdSigEd25519(int t, int n) {
        this.t = t;
        this.n = n;
    }
    ...
    public List<EdwardsPoint> gatherRi(ThresholdSigEd25519Params params, String toSign, Set<Integer> nodes) throws NoSuchAlgorithmException {
        ...
        for (int i = 0; i < n; i++) {
            if (nodes.contains(i)) {
                ...
                Scalar rsi = computeRs(privateShare, toSign);

                rs.add(rsi);
                EdwardsPoint res = mulBasepoint(rsi);
                ri.add(res);
            }
        }
